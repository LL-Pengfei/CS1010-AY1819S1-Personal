# Exercise 5: Square, Escape, Magic
...
- Inside that directory, you should see a bunch of files:
    - `square.c`, `escape.c`, `magic.c` are the most important files.  They are the weitsang C code that you should edit to solve the exercise.
...
Write a progrma `square`, that reads from standard inputs, an integer `n`, and prints to standard output the expected pattern of squares.
...
Robot Asimo is lost in a maze that contains traps!  Luckily you can control him remotely to help him escape.
...
To remotely control Asimo, you can give Asimo a series of instructions denoted by `U`,`D`,`L`, or `R`. `U` instructs Asimo to move to the cell one row above. `D` instructors Asimo to go to the cell one row below.  `L` instructs Asimo to move to the cell one column to the left; and `R` instructs Asimo to move to the cell one column to the right.  Asimo follows the instructions in the order they are given, and it only moves one cell adjacent by following each instruction, with below exceptions:

1. If an instruction leads Asimo to a wall, Asimo's path is blocked and it will stay in position.  It continues with the next instruction if there is one.

2. if an instruction leads Asimo to a trap, it is trapped there and unable to move anymore.  All subsequent instructions have no effect on Asimo.  Poor Asimo!

3. If the instruction leads Asimo to anywhere outside the maze, Asimo escapes happily and any instructions after that is ignored.

For conveniece, we denote the row indices as 0 to $m$-1 from top to bottom, and column indices as 0 to $n$-1 from left to right.

Write a program `escape`, that reads from standard input two positive integer $m$ and $n$. Then reads from standard input $m$ rows with $n$ characters in each row. These characters denote the maze of size $m \times n$. Each character can only be `#`, `.`, `*` or `A`, where `A` denotes the initial position of Asimo on an empty space.  It is guaranteed only one `A` exists in the input.  Your program then reads a string denoting the order you give to Asimo.  Each character can only be `U`, `D`, `L`, or `R`.  Finally, your program prints, to the standard output, one of the following:

1. If Asimo escaped from the maze after following some instructions, print "ESCAPED!"
2. If Asimo is trapped due to your unwise instruction, print "TRAPPED!"
3. If Asimo is still stuck in the maze but not trapped after following all the instructions, print the position of Asimo as `x y` where `x` and `y` denotes the row and column of Asimo's position after following all your instructions.
...
A magic square is a grid of $n \times n$ with each cell filled with distinct number from 1 to $n^2$, such that, the sum of each row, column or diagonal are all the same. 

Write a program, `magic`, that construct a magic square of size $n \times n$, where $n$ is odd.

There are different ways we can construct such as magic square.  For this question, you shall follow the following algorithm:

1. Put 1 at center column of top row
2. Every other numbers are placed one row up and one column to the right of the previous number.  This may not always work when we reach the edge of the square or when the cell one row up and one column to the right is occupied.  In such case, you should follow the rules below:
    ...
    - C. If a number's cell is outside of rightmost column, stay in that row and put it in the leftmost column;
...
The number 2 is filled above the top row, so we apply Rule B.  The number 4 is fileld outside the rightmost column, so we apply Rule C.
