...
- Inside that directory, you should see a bunch of files:
    - `square.c`, `escape.c`, and `magic.c` are the most important files.  They are the C code that you should edit to solve the exercise.
...
A magic square is a grid of $n \times n$ with each cell filled with a distinct number from 1 to $n^2$, such that, the sum of each row, column or diagonal are all the same. 
