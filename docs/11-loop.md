...
Some times, we want to execute one of the branch repeatedly, over-and-over again, until certain logical condition is met.  You have seen this construct in the $max$ and $sum$ example, where we repeatedly go through the numbers in the list until $i == k$.
...
- what do we need to setup before repeating the above?
- what would be different each time we repeat the above?  
- how to decide if we should stop repeating (or conversely, to continue repeating?)

There is a fifth question that is as important as the above, but is harder, so we will leave it for the moment.

Let's the example of `max`.  Here is the answer to the questions:

- What do we want to do repeatedly?  We want to compare the element $l_i$ to $m$, and update $m$ to $l_i$ if $l_i > m$.

- What do we need to setup before repeating the above?  We need to have some initial value of $m$ and $i$.  We set it up as $m = l_0$ and $i = 1$.
...
- What do we need to setup before repeating the above?  We setup the partial product to be $n$ and $i$ to be $n - 1$.
- What would be different each time we repeat the above?  $i$ would decrease by one every time we repeat the above.
...
- The `initialize` statement set up the loop (i.e., "What do we need to setup before repeating the above?").  
- The `condition` statement indicates when we should stop repeat.  If it evaluates to `true`, it will cause the loop to continue, otherwise, the loop stops.
- The `update` statement is executed after every loop.  (i.e., "What would be different each time we repeat the above?")
- The `body` statement is the action that we want to do repeatedly.  

![for](figures/loops/loops.003.png)

For the `initialize` and `update` statement, even though C allows us to include mulitple initializations and mulitple updates, it makes the code harder to read and understand.  C also allows us to have empty statement for `initialize`, `condition` and/or `update`.  As such, in CS1010, we will stick to performing one initialization and one update only.

For example, in the setup phase for factorial, we should initialize the partial product to $n$ and $i$ to $n - 1$, that's two assignments.  We commonly choose the index which we increment or decrement as we loop, hence we choose to initialize $i$ in the `initialize` statement of the `for` loop, and left initialization of `product` to a separate assignment before the for loop.
...
Note that the syntax of the `while` loop above does not contain the `<initialize>` and the `<update>` component.  When we write the loop, however, we still need to think about them, and add them at the appropriate place in the code, as follows:
...
Let's write a program that plays a game with the user.  The program generates a random integer number between 1 and 100.  The user will guess what that number is.  If the user guesses correctly, the program congratulates the user.  Otherwise, the program tells the user whether the guess is too high or too low, and lets the user guess again.  This process repeats until the guess is correct.
...
- What do we need to setup before repeating the above?  We need to generate a random integer between 1 and 100.
- What would be different each time we repeat the above?  The guess _may_ be different.
...
I will not go into details of what `srandom(times(0))` means.  You can look it up on your own (InfoSec students may want to do this as generating good random number is key to keeping systems secure).  
...
(C) Extend the "Guess A Number" program so that it plays the game for five rounds with the user, and at the end, shows the user the average number of guesses over five rounds.  (Hint: you should put the loop that reads the guess and prints feedback to user into another function.)
