# Assignment 5: SelectionSort, Add, Mastermind
...
- Edit the files `sxxxxxxxxx`, to solve the corresponding question as described below.
...
The files `sxxxxxx`, will be uploaded to GitHub.  You can submit multiple times, but only the last submission will be graded.
...
4. In each iteration, all cells will remain or switch their status, according to below rules by referring to the universe of previous iteration:
    a. Any live cell with fewer than two live neighbors becomes dead
    b. Any live cell with two or three live neighbors remains alive.
    c. Any live cell with more than three live neighbors becomes dead.
    d. Any dead cell with exactly three live neighbors becomes alive
    e. **Any border cell becoming alive (row/column 0 or n-1), is going to be dead in next iteration regardless of rule a, b, c or d. This is to simplify the game and restrict the size of the universe. This means border cell may become alive according to above rules, but it will only survive for 1 iteration**

You may verify the rules by checking how the pattern during each iteration in below image. The pattern is called glider and it will move indefinitely in one direction. Here shows status of 5 iterations only. The number in each cell denotes how many of its neibouring cells are alive. After 4 iterations, the glider moves 1 cell to the right and to the bottom.
![glider](figures/glider.png)
<small>[source](http://www.paleotechnologist.net/?p=451)<small>

Write a program `gameoflife`. It reads from standard inputs 2 integers n and k in first row where n denotes the universe is a n*n grid, and k is the number of iterations to simulate. It then reads from standard inputs n rows, with n characters in each row representing the inital state. Each character is either alive ("#") or dead ("."). 
...
1. if the order lead Ali to a wall, he will be angry and stay in position, and continue with next order if there is
2. if the order lead Ali to a trap, he is trapped there and unable to move anymore. OOPS POOR ALI
3. if the order lead Ali to anywhere outside the maze, Ali escaped happily and any order after that is ignored

Write a program `maze`, that reads from standard input two integer m and n (3<=m,n<=100). Then reads from standard input m rows with n characters in each row. This denotes the maze of size m*n. Each character will only be "#", ".", "*" or "A" where "A" denotes initial position of Ali and it is an empty space. Then read one row of string denoting the order you give to Ali, and each character is only "U","D","L" or "R". Print to standard output one of the following:
...
2. All the other numbers are placed 1 row up and 1 column to the right of the previous number. This may have some problems, and you shall follow below rules to resolve them.
    a. If a number's position is already taken, put it 1 row below the position of the previous number
    b. If a number's position is above the top row, stay in that column and put it in the bottom row
    c. If a number's position is outside of rightmost column, stay in that row and put it in the leftmost column
    d. If the number's position is outside both a row and a column, put it 1 row below the previous number
