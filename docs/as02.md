...
Now, to visualize the distribution of primes, we do the following, for each cell of the triangle that contains at least one prime, we print `#` to the standard output at the corresponding position.  Otherwise, we print ` ` (a white space).
