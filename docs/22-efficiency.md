...
How much is the speed up in the worse case?  In the input above, with the original slow code, in the worst case, I need to check through ~10,000,000,001 divisors.  With the faster version, I only need to check ~$\sqrt(10,000,000,001)$ = ~10,000 divisors.  That's where the five orders of magnitude speedup come from.
